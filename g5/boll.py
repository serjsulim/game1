...
class Boll:
    def __init__(self, screen):
        ...
        self.rect = pygame.Rect(WIDTH//2, HEIGHT - 35, RADIUS_BOLL, RADIUS_BOLL) # створюємо прямокутник для м'яча
        self.sound_boll = pygame.mixer.Sound("music/2.wav")
    ...
    def update(self, raketka):           # слідкуємо за рухом м'яча
        ...
        if self.rect.colliderect(self.raketka): # якщо м'яч доторкнувся до ракетки
            self.sound_boll.play()
            self.rect.y -= 7                    # виправлення багу щоб м'яч не застрягав у ракетці
            self.speed_y *=  choice((-1.1, -1,  -0.9))    # відбиття м'яча
